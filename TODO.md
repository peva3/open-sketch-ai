...
- [x] **T-6.1** Optional per-user config file (git-ignored) to store named **provider profiles** (OpenAI, Azure, OpenRouter, Groq, Ollama, LM Studio, vLLM, Unsloth) with `base_url`/`api_key`/`model`/`dialect`; secrets never committed. Implemented as `agent/profiles.py`: `profiles.toml` lives in the per-OS user config dir (`profiles.config_dir()`: `SUPEX_AI_CONFIG_DIR` override > Windows `%APPDATA%` > macOS `~/Library/Application Support` > `$XDG_CONFIG_HOME`/`~/.config`, each under `supex-chat/`), parsed with stdlib `tomllib`, optional top-level `default = "name"`. `load_config(profile=...)` resolution order flags > `SUPEX_AI_*` env > `OPENAI_*`/`ANTHROPIC_*` env > profile mapping > built-in defaults (env always beats profile); `SUPEX_AI_DIALECT` env now honored; `SUPEX_AI_PROFILE` env + `--provider-profile` flag both wired in the CLI. 33 new tests (config profile class + `test_profiles.py` + CLI routing). Full driver suite **949 passed** (was 917).
